...
<Note>
The default mode is `read` (SELECT and functions). Pass `--access read-call` to run stored procedures, or `--access write` for INSERT, UPDATE, DELETE, and DDL. A system's `access` in `~/.ibmi/config.yaml` is a ceiling for that system, never a default: `--access write` against a system configured as `read` runs as `read`, and a system configured as `write` still runs as `read` unless you pass `--access write`.
</Note>

<Warning>
**`IBMI_EXECUTE_SQL_ACCESS` in your environment is a ceiling.** If it is set lower than the mode you request, the command exits with code 4 and a message naming the variable instead of letting Db2 reject the statement later. Check your shell and any `.env` in the current directory. See [execute_sql access mode](/configuration#execute_sql-access-mode).
</Warning>
