...
<Warning>
**`IBMI_EXECUTE_SQL_READONLY=true` in your environment overrides `--no-read-only`.** An explicit `true` pins read-only mode. The flag is accepted, but write statements are still rejected at the connection. If `--no-read-only` has no effect, check your shell and `.env` for the variable. See [Read-Only Policy Precedence](/configuration#read-only-policy-precedence).
</Warning>
