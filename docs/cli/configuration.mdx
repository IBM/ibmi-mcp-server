...
| `access` | `read` \| `read-call` \| `write` | — | Ceiling for `execute_sql` on this system: `read` (SELECT and functions), `read-call` (adds `CALL`), `write` (everything). Unset means no ceiling; `ibmi sql --access` (default `read`) decides |
| `readOnly` | boolean | — | Deprecated alias for `access`: `true` → `read`, `false` → `write` |
