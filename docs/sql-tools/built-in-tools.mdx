...
1. **In-process validation** via `SqlSecurityValidator` — classifies the statement with the vscode-db2i parser, enforces read-only mode constraints and query length
2. **Native IBM i validation** via `QSYS2.PARSE_STATEMENT` — runs when `IBMI_EXECUTE_SQL_PARSE_VALIDATION=always` (strict/audit mode), or as a fallback when the in-process parser could not classify the statement. Under the default `auto` mode, classified statements skip this wire round trip for lower latency
3. **JDBC connection backstop** — when read-only mode is in effect, the connection pool is created with `access=read call`, so the IBM i JDBC driver itself rejects INSERT/UPDATE/DELETE/DDL at the connection level (CALL remains allowed for `describe_sql_object`). An explicit `access` value in `DB2i_JDBC_OPTIONS` overrides this default

Only layer 1 runs on every query. Layer 2 is [mode-dependent](/configuration#parse-validation-modes), and layer 3 applies to the connection pool shared by `execute_sql` and the built-in tools — **YAML source pools are not affected** and keep their own `jdbc-options`.
...
Full reference: [Read-Only Access Backstop](/configuration#read-only-access-backstop) and [Read-Only Policy Precedence](/configuration#read-only-policy-precedence).
</Note>

<Warning>
Every layer checks statement type, not effect — a SELECT that invokes a side-effecting function (for example `QSYS2.QCMDEXC`) passes all three and runs. Read-only mode is a guardrail against accidents, not a security boundary. Where read-only must be guaranteed, run the server under a least-privilege IBM i user profile — object authority is the enforced boundary.
</Warning>
...
| `IBMI_EXECUTE_SQL_READONLY` | `true` | When `true`, `execute_sql` only allows SELECT queries. Set to `false` to permit data modification statements. Parsed fail-closed — only `false` or `0` disables it. |
| `IBMI_EXECUTE_SQL_PARSE_VALIDATION` | `auto` | When to run the `QSYS2.PARSE_STATEMENT` wire validation. `auto` skips it when the in-process parser classified the statement; `always` runs it for every query. |
...
<Note>
Setting `IBMI_EXECUTE_SQL_READONLY=true` **explicitly** also pins read-only mode: the connection stays read-only even if a YAML tool or CLI flag requests write access at runtime. Leaving it unset produces the same default but allows those runtime overrides. See [Read-Only Policy Precedence](/configuration#read-only-policy-precedence).
</Note>
...
# Strict/audit mode — validate every statement with IBM i's own parser
IBMI_EXECUTE_SQL_PARSE_VALIDATION=always npx -y @ibm/ibmi-mcp-server@latest --builtin-tools --execute-sql
```
