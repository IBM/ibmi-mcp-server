...
<Warning>
All three layers classify the statement. None of them inspect what it invokes. A SELECT that calls `QSYS2.QCMDEXC` passes every layer and runs in read-only mode. Read-only mode is not a security boundary. The boundary is the IBM i user profile's object authority; run the server under a least-privilege profile.
</Warning>
