...
`execute_sql` applies layered validation before running any query:

1. **In-process validation** via `SqlSecurityValidator` — classifies the statement with the vscode-db2i parser and rejects anything outside the configured access mode; also enforces query length
2. **Native IBM i validation** via `QSYS2.PARSE_STATEMENT` — runs only when the in-process parser could not classify the statement, and applies the same per-mode allow-list (`QUERY` in `read`, `QUERY` and `CALL` in `read-call`)
3. **JDBC connection backstop** — the connection pool is created with `access=read call` in `read` and `read-call` mode (Db2 rejects INSERT/UPDATE/DELETE/DDL at the connection; CALL stays open for `describe_sql_object`) and `access=all` in `write` mode. An explicit `access` in `DB2i_JDBC_OPTIONS` overrides this value

Layer 1 runs on every query, layer 2 only as a fallback, and layer 3 applies to the connection pool shared by `execute_sql` and the built-in tools — **YAML source pools are not affected** and keep their own `jdbc-options`.

<Note>
The mode is `IBMI_EXECUTE_SQL_ACCESS`: `read` (default, SELECT and functions), `read-call` (adds `CALL`), or `write` (everything). Full reference: [execute_sql access mode](/configuration#execute_sql-access-mode).
</Note>

<Warning>
All three layers classify the statement. None of them inspect what it invokes. A SELECT that calls `QSYS2.QCMDEXC` passes every layer and runs in `read` mode. The access mode is not a security boundary. The boundary is the IBM i user profile's object authority; run the server under a least-privilege profile.
</Warning>
...
| `IBMI_EXECUTE_SQL_ACCESS` | `read` | What `execute_sql` may run: `read` (SELECT and functions), `read-call` (adds `CALL`), `write` (everything). Unrecognized values fall back to `read`. Replaces the deprecated `IBMI_EXECUTE_SQL_READONLY`. |
...
<Note>
Setting `IBMI_EXECUTE_SQL_ACCESS` **explicitly** makes it a ceiling: a YAML tool or `ibmi sql --access` can lower the mode at runtime but never raise it, and the CLI reports an error instead of silently downgrading. Leaving it unset produces the same `read` default but lets the runtime decide. See [execute_sql access mode](/configuration#execute_sql-access-mode).
</Note>
...
# Allow CALL to stored procedures (writes still blocked)
IBMI_EXECUTE_SQL_ACCESS=read-call npx -y @ibm/ibmi-mcp-server@latest --builtin-tools --execute-sql

# Enable write access for execute_sql
IBMI_EXECUTE_SQL_ACCESS=write npx -y @ibm/ibmi-mcp-server@latest --builtin-tools --execute-sql
```
...
*`execute_sql` annotations follow `IBMI_EXECUTE_SQL_ACCESS`: in `read` (default) `readOnlyHint=true` and `destructiveHint=false`; in `read-call` and `write` `readOnlyHint=false` and `destructiveHint=true`, because a `CALL` can mutate data.
