...
<Warning>
Client-side validation is a guardrail, not a security boundary: no client-side check can see inside a stored procedure or user-defined function. For deployments where read-only must be guaranteed, run the server under a least-privilege IBM i user profile — the profile's object authority is the enforced boundary.
</Warning>
