...
<Note>
**YAML sources are not governed by the execute_sql access mode.** The server derives JDBC `access` from `IBMI_EXECUTE_SQL_ACCESS` only for the pool shared by `execute_sql` and the built-in tools. Pools created from YAML sources use exactly the `jdbc-options` you configure. To restrict a YAML source's connection the same way, set `access` on that source explicitly. See [execute_sql access mode](/configuration#execute_sql-access-mode).
</Note>
