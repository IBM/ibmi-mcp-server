...
<Note>
**YAML sources are not subject to the read-only access backstop.** When `execute_sql` runs in read-only mode, the server injects `access=read call` into the connection pool used by `execute_sql` and the built-in tools. Pools created from YAML sources are unaffected and use exactly the `jdbc-options` you configure. To restrict a YAML source's connection the same way, set `access` on that source explicitly. See [Read-Only Access Backstop](/configuration#read-only-access-backstop).
</Note>
