...
| `ignore-unauthorized` | boolean | No | `false` | Skip Mapepire TLS verification (allow insecure TLS). When omitted, `DB2i_IGNORE_UNAUTHORIZED` applies |
...
| `ignore-unauthorized` | boolean | `false` | Skip Mapepire TLS verification (allow insecure TLS). When omitted, `DB2i_IGNORE_UNAUTHORIZED` applies |
