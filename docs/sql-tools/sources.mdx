...
<Tabs>
  <Tab title="Port Configuration">
    ...
    **Environment variable (same value as server `.env`):**
    ```yaml
    sources:
      ibmi-system:
        host: ${DB2i_HOST}
    ...
    <Note>
    Interpolation supports `${VAR_NAME}` only — there is no `${VAR:default}` syntax. If `DB2i_PORT` is unset at YAML load time, the placeholder stays literal and validation fails; omit `port` instead to use Mapepire's default **8076**, or set `DB2i_PORT` in `.env` before starting the server.
    </Note>
  </Tab>
