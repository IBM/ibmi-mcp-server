...
**Alternative: persist variables in your shell profile**
...
Local development can load a dotenv file only when `MCP_SERVER_CONFIG` points at it.
...
<Tip>
Export variables in your shell for persistent configuration in the current session:
```bash
export DB2i_HOST=ibmi-dev.mycompany.local
export DB2i_USER=DEVUSER
export DB2i_PASS=DevPassword123
...
export YAML_ALLOW_DUPLICATE_SOURCES=true
```
The server reads the process environment. It does not load a `.env` file unless `MCP_SERVER_CONFIG` is set.
</Tip>
...
<AccordionGroup>
  <Accordion title="Development vs Production">
    **Development:**
    ...
    - Set `YAML_ALLOW_DUPLICATE_SOURCES=true`
