...
```bash
# Load single toolset
npx ibmi-mcp-server --tools tools/my-tools.yaml --toolsets performance_monitoring

# Load multiple toolsets (comma-separated)
npx ibmi-mcp-server --tools tools/my-tools.yaml --toolsets performance_monitoring,security_audit

# Load from environment variable
SELECTED_TOOLSETS=employee_information,payroll_processing
npx ibmi-mcp-server --tools tools/my-tools.yaml --toolsets $SELECTED_TOOLSETS
```
...
npx ibmi-mcp-server --list-toolsets --tools tools/my-tools.yaml
```
...
npx ibmi-mcp-server --tools tools/ --toolsets performance_monitoring,security_audit
```
...
**Start server:**
```bash
npx ibmi-mcp-server --tools tools/ --toolsets $SELECTED_TOOLSETS
```
