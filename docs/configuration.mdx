...
| `access` | Final override. The pool shared by `execute_sql` and the built-in tools normally derives JDBC `access` from [`IBMI_EXECUTE_SQL_ACCESS`](#execute_sql-access-mode); an explicit value here replaces it and is logged as a warning |

<Note>
**`access` overrides the JDBC value only.** The SQL validator still enforces the access mode. `access=all` with mode `read` removes the connection-level backstop but leaves the validator in place; `access=read only` with mode `write` makes Db2 reject statements the validator allowed, which surfaces as a driver error. Use the override deliberately.
</Note>
...
#### JDBC `access` and the execute_sql access mode

**The pool shared by `execute_sql` and the built-in tools derives its Toolbox JDBC `access` property from [`IBMI_EXECUTE_SQL_ACCESS`](#execute_sql-access-mode):** `read` and `read-call` connect with `access=read call`, `write` with `access=all`. An explicit `access=` in `DB2i_JDBC_OPTIONS` is the final override and is logged as a warning. YAML source pools are unaffected and use exactly the `jdbc-options` you configure.

<Warning>
**Access modes restrict statement types, not effects.** The in-process validator, `PARSE_STATEMENT`, and the JDBC `access` setting each classify the submitted statement. None of them inspect what it invokes. `SELECT QSYS2.QCMDEXC('...') FROM SYSIBM.SYSDUMMY1` is a query to all three, so the CL command runs even in `read` mode. The security boundary is the IBM i user profile. Run the server under a profile with least-privilege object authority.
</Warning>
...
| `execute_sql` | ⚠️ Disabled by default | Execute ad-hoc SQL queries (`read` mode by default) | `--execute-sql` | `IBMI_ENABLE_EXECUTE_SQL` |
...
| `IBMI_EXECUTE_SQL_ACCESS` | What `execute_sql` may run — see [execute_sql access mode](#execute_sql-access-mode) | `read` | `read`, `read-call`, `write` |

<Warning>
**Security**: `execute_sql` lets clients run ad-hoc SQL. By default (`read`) only SELECT statements are accepted, enforced in-process and by the JDBC `access` property on the connection. `read-call` adds `CALL`; `write` allows everything and belongs in development only. These checks restrict statement types and cannot stop a query that invokes a side-effecting function such as `QSYS2.QCMDEXC`. The security boundary is the IBM i user profile. In production, use curated YAML tools and a least-privilege profile.
</Warning>
...
<Tabs>
  ...
  <Tab title="Procedures (read-call)">
    ```bash
    IBMI_EXECUTE_SQL_ACCESS=read-call
    npx -y @ibm/ibmi-mcp-server@latest --builtin-tools --execute-sql
    ```

    **Use for**: Running stored procedures while INSERT/UPDATE/DELETE/DDL stay blocked
  </Tab>
...
<AccordionGroup>
  ...
  <Accordion title="execute_sql (--execute-sql)" icon="code">
    ...
    - **Access mode**: `IBMI_EXECUTE_SQL_ACCESS` is `read` (default), `read-call`, or `write` — see [execute_sql access mode](#execute_sql-access-mode)
    - **In-process classification**: The vscode-db2i parser classifies each statement locally and rejects anything outside the mode, with regex pattern matching as a fallback
    - **PARSE_STATEMENT fallback**: IBM i's native SQL parser (`QSYS2.PARSE_STATEMENT`) runs only when local classification is inconclusive and applies the same allow-list
    - **JDBC access backstop**: the connection is created with `access=read call` (`read`, `read-call`) or `access=all` (`write`), so the driver refuses writes in the first two modes
...
**`IBMI_EXECUTE_SQL_ACCESS` is the one setting that decides what `execute_sql` may run.** It configures the in-process SQL validator, the `PARSE_STATEMENT` fallback, and the JDBC `access` property of the connection pool together, so every layer enforces the same rule and a blocked statement always fails with a validation error that names the mode.

| Mode | The tool accepts | `PARSE_STATEMENT` allows | JDBC `access` on the shared pool |
|------|------------------|--------------------------|----------------------------------|
| `read` **(default)** | SELECT, CTEs, table and scalar functions | `QUERY` | `read call` |
| `read-call` | `read` plus `CALL` to stored procedures | `QUERY`, `CALL` | `read call` |
| `write` | Every statement the connection's user profile allows | unchecked | `all` |

```bash
# Default — SELECT and functions only
IBMI_EXECUTE_SQL_ACCESS=read

# Run stored procedures, still no INSERT/UPDATE/DELETE/DDL
IBMI_EXECUTE_SQL_ACCESS=read-call
...
<Note>
**Why `read` still connects with `access=read call`**: the pool is shared with `describe_sql_object`, which runs `CALL QSYS2.GENERATE_SQL`. The in-process parser and `PARSE_STATEMENT` are what keep `CALL` out of `read` mode; the JDBC layer guarantees that no write statement reaches Db2 in either non-write mode.
</Note>

#### Validation layers

1. **In-process classification** (every query): the vscode-db2i parser classifies each statement and rejects anything outside the mode. If the parser cannot classify the input, a regex keyword check runs instead.
2. **`QSYS2.PARSE_STATEMENT`** (fallback only): runs when the local parser could not classify the statement, for example on comment-only input or unusual syntax, and applies the allow-list from the table. Classified statements skip this round trip.
3. **JDBC `access`** (connection level): Db2 itself rejects INSERT/UPDATE/DELETE/DDL on a `read call` connection, even if a statement slipped past the first two layers. An explicit `access=` in `DB2i_JDBC_OPTIONS` overrides this value.

#### Precedence: the environment is a ceiling

Runtime configuration can also set the mode: `ibmi sql --access <mode>`, a system's `access` in `~/.ibmi/config.yaml`, a YAML tool's `security.readOnly`, or `configureExecuteSqlTool()` in code. Whether the environment or the runtime wins depends on whether the variable was set explicitly:

| Environment | Runtime request | Effective mode |
|-------------|-----------------|----------------|
| **Unset** | any | the runtime request (default `read`) |
| `IBMI_EXECUTE_SQL_ACCESS=read` | `read-call` or `write` | `read` — the CLI reports an error naming the variable |
| `IBMI_EXECUTE_SQL_ACCESS=read-call` | `write` | `read-call` |
| `IBMI_EXECUTE_SQL_ACCESS=read-call` | `read` | `read` — lowering is always allowed |
| `IBMI_EXECUTE_SQL_ACCESS=write` | any | the runtime request |

<Warning>
**Unset and `=read` are not equivalent.** Both default to `read`. Only an explicit value is a ceiling. If a deployment must never accept CALL or write statements, set `IBMI_EXECUTE_SQL_ACCESS=read`. If the `ibmi` CLI should decide per command, leave the variable unset — a copied `.env.example` in the working directory is the most common reason `ibmi sql --access write` reports that access is capped.
</Warning>

#### Value parsing

Values are trimmed and lowercased. Anything other than `read`, `read-call`, or `write` falls back to `read` and logs a warning to stderr; a strict enum would fail the entire environment config on a typo.

#### Migrating from `IBMI_EXECUTE_SQL_READONLY`

`IBMI_EXECUTE_SQL_READONLY` is deprecated and still honored for one release with a stderr notice. When both variables are set, `IBMI_EXECUTE_SQL_ACCESS` wins.
...
| `DB2i_JDBC_OPTIONS='access=...'` | still honored as the final override of the JDBC value; usually no longer needed |
| `ibmi sql --read-only` / `--no-read-only` | `ibmi sql --access read` / `--access write` |
| `readOnly: true` on a CLI system | `access: read` |
