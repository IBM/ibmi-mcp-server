...
<Warning>
Set credentials in the process environment (shell `export`, Docker `-e`, systemd, Kubernetes secrets). Never commit sensitive credentials to version control. A dotenv file is for local development only and is loaded only when `MCP_SERVER_CONFIG` points at it.
</Warning>
...
The server reads configuration from the process environment. Export variables in your shell, or inject them with Docker `-e`, systemd, or Kubernetes secrets.
...
<Note>
**Local development only:** To load variables from a file, set `MCP_SERVER_CONFIG` to that path (`MCP_SERVER_CONFIG=.env`). The server does not search for `.env` unless this variable is set. CLI arguments (like `--transport http`) override environment variables.
</Note>
...
Export the minimal configuration above in your shell, or inject the same variables through your process supervisor. `.env.example` is a variable catalog for local development; it is not loaded unless you set `MCP_SERVER_CONFIG`.
...
<AccordionGroup>
  <Accordion title="Development & Testing → Use 'none'" icon="flask">
    ...
    **Setup:** Simply set `MCP_AUTH_MODE=none`
  </Accordion>
...
<Steps>
  ...
  <Step title="Configure Environment Variables">
    Set the keypair paths in the process environment:
...
<AccordionGroup>
  ...
  <Accordion title="Version Control" icon="code-branch">
    ...
    - **Never commit credentials**: Keep secrets in the process environment or a secrets manager
...
<Warning>
**Path Requirements:**
- `TOOLS_YAML_PATH` should use **absolute paths** to avoid issues when the server is started from different directories
