...
<Tabs>
  <Tab title="Readonly Mode (Default)">
    ```bash
    IBMI_ENABLE_EXECUTE_SQL=true
    IBMI_EXECUTE_SQL_READONLY=true  # Default - only SELECT queries
    ```

    **Use for**: Development exploration, debugging SELECT queries, safe ad-hoc queries
  </Tab>

  <Tab title="Write Mode (Opt-in)">
    ```bash
    IBMI_ENABLE_EXECUTE_SQL=true
    IBMI_EXECUTE_SQL_READONLY=false  # Allow INSERT/UPDATE/DELETE
    ```
