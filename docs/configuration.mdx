...
<Note>
**Why empty values are ignored**: `access=` would otherwise forward an empty string to the driver *and* suppress the [read-only access backstop](#read-only-access-backstop) — silently weakening security through what looks like a no-op. A dangling pair is treated as if the key were never present.
</Note>
...
**When `execute_sql` runs in read-only mode, the server sets JDBC `access=read call` on its connection pool** — so the IBM i JDBC driver rejects writes at the connection level, independent of any client-side SQL inspection. This is a defense-in-depth backstop: even if a statement slips past in-process validation, the connection itself cannot perform the write.
...
<Warning>
`DB2i_JDBC_OPTIONS='access=all'` disables the connection-level backstop, leaving only in-process validation between a client and a write. Set it only when write access is intended.
</Warning>
...
<Warning>
**Security**: `execute_sql` allows clients to run ad-hoc SQL queries. By default (`IBMI_EXECUTE_SQL_READONLY=true`), only SELECT queries are allowed — enforced in-process by the SQL validator and at the connection level by a Toolbox JDBC `access=read call` backstop on the built-in tools pool. Set `IBMI_EXECUTE_SQL_READONLY=false` to enable write operations (INSERT, UPDATE, DELETE). Enable only in development or trusted environments. Production deployments should use YAML-defined tools for controlled query access, and run the server under a least-privilege IBM i user profile — the profile is the enforced security boundary; client-side checks cannot see inside stored procedures or UDFs.
</Warning>
...
<AccordionGroup>
  ...
  <Accordion title="execute_sql (--execute-sql)" icon="code">
    ...
    - **JDBC access backstop**: Read-only mode sets `access=read call` on the connection, so writes are refused by the driver
...
**`IBMI_EXECUTE_SQL_PARSE_VALIDATION` controls whether `execute_sql` makes a second round trip to IBM i to validate each statement.** Before running a query, the server classifies it in-process with the vscode-db2i parser. Historically it then *also* called `QSYS2.PARSE_STATEMENT` on the wire — doubling the round trips for every query, even when the local parser had already produced an unambiguous answer.
...
<Info>
**Why skipping is safe**: the read-only guarantee does not rest on `PARSE_STATEMENT`. In-process validation rejects writes before execution, and the [JDBC `access=read call` backstop](#read-only-access-backstop) means the connection itself cannot perform one. `PARSE_STATEMENT` adds authoritative statement typing from IBM i, which is valuable for audit trails but is not the enforcement boundary.
</Info>

<Tip>
**Choose `always` when** you need every statement independently typed by IBM i for compliance or audit logging, or when you are debugging a statement the local parser classifies incorrectly. **Choose `auto`** (the default) for everything else — the latency saving is most noticeable on remote or high-latency IBM i connections.
</Tip>

<Note>
**Unrecognized values fall back to `auto`** and log a warning to stderr. This is deliberate: a strict enum would fail whole-environment validation on a typo and reset unrelated settings to their defaults. Because a typo silently downgrades strict mode, the warning is written to stderr unconditionally so it appears in stdio and container logs — check for it after setting `always`.
</Note>

### Read-Only Policy Precedence

**`IBMI_EXECUTE_SQL_READONLY` can be either a default or a hard floor, depending on whether you set it explicitly.** Read-only mode is seeded from the environment but can also be changed at runtime — by a YAML tool declaring `security.readOnly: false`, or by CLI commands such as `ibmi sql --no-read-only`. Which one wins depends on how the variable was supplied:
...
<Warning>
**Unset and `=true` are not equivalent.** Both give you read-only behavior out of the box, but only the explicit `=true` is enforced as a floor. If your goal is "this deployment can never write," set it explicitly — leaving it unset lets a shared YAML tool declaring `security.readOnly: false` strip the guarantee, including the [JDBC access backstop](#read-only-access-backstop).
</Warning>

<Note>
**Where the pin is enforced**: the pin keeps `access=read call` on the connection pool. A runtime override can still relax the *in-process* validator, so a write may pass client-side checks and then be rejected by the IBM i driver — surfacing as a database error rather than a validation error. The write does not succeed either way; only the error message differs.
</Note>
...
<Note>
This differs from the server's other boolean variables, which are enabled only by an explicit `true`/`1`. Because this flag gates write access and seeds the JDBC backstop, an unrecognized value must not silently open write access. If you previously used a non-canonical value such as `yes` to mean write mode, it now resolves to **read-only** — use `false`.
</Note>
