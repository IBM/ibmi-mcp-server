...
| `DB2i_IGNORE_UNAUTHORIZED` | Skip Mapepire TLS certificate verification (allow insecure TLS). YAML `ignore-unauthorized` wins when set; otherwise this env applies | `false` | `true` | ⚠️ **High Risk**: Keep `false` in production |
