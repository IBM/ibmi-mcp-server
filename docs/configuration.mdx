...
<Note>
**Why empty values are ignored**: `access=` would otherwise forward an empty string to the driver *and* suppress the [read-only access backstop](#read-only-access-backstop) — a silent security downgrade from what looks like a no-op.
</Note>
...
**When `execute_sql` runs in read-only mode, the server sets JDBC `access=read call` on its connection pool.** The IBM i JDBC driver then rejects write statements (INSERT, UPDATE, DELETE, DDL) at the connection level, even if one slips past in-process validation.
...
<Warning>
**Read-only mode restricts statement types, not effects.** Every layer — the in-process validator, `PARSE_STATEMENT`, and the JDBC `access` setting — checks what kind of statement was submitted, not what it invokes. A query that calls a side-effecting function, such as `SELECT QSYS2.QCMDEXC('...') FROM SYSIBM.SYSDUMMY1`, is a SELECT to all three layers and runs even under `access=read only`. Treat read-only mode as a guardrail against accidents. The security boundary is the IBM i user profile: run the server under one with least-privilege object authority.
</Warning>
...
<Warning>
**Security**: `execute_sql` lets clients run ad-hoc SQL. By default (`IBMI_EXECUTE_SQL_READONLY=true`) only SELECT statements are accepted, enforced in-process and by a [JDBC `access=read call` backstop](#read-only-access-backstop) on the connection. Set `IBMI_EXECUTE_SQL_READONLY=false` to allow writes — development and trusted environments only. These checks restrict statement types; they cannot stop a query that invokes a side-effecting function such as `QSYS2.QCMDEXC`. Run the server under a least-privilege IBM i user profile — the profile is the security boundary — and prefer curated YAML tools in production.
</Warning>
...
<Info>
**Why skipping is safe**: read-only enforcement does not rest on `PARSE_STATEMENT`. Write statements are rejected in-process and by the [JDBC backstop](#read-only-access-backstop); `PARSE_STATEMENT` adds authoritative statement typing from IBM i — valuable for audit, but the same type-level check the other layers already make.
</Info>

<Tip>
**Choose `always`** for compliance or audit logging, or when debugging a statement the local parser misclassifies. **Choose `auto`** (the default) otherwise — the saving matters most on high-latency connections.
</Tip>

<Note>
**Unrecognized values fall back to `auto`** and log a warning to stderr (a strict enum would fail the entire environment config on a typo). Because a typo silently downgrades strict mode, check for the warning after setting `always`.
</Note>
...
<Warning>
**Unset and `=true` are not equivalent.** Both default to read-only, but only an explicit `=true` is a floor. If the deployment must never accept write statements, set it explicitly — when unset, a YAML tool or CLI flag can enable writes at runtime and drop the [JDBC backstop](#read-only-access-backstop) with them.
</Warning>

<Note>
**Where the pin is enforced**: at the connection — the pin keeps `access=read call` on the pool. A runtime override can still relax the in-process validator, so a blocked write surfaces as a database error from the driver instead of a validation error. Either way the write statement does not run.
</Note>
...
<Note>
This is the opposite of the server's other boolean variables, which enable only on `true`/`1` — a flag that gates write access must not open it on an unrecognized value. If you previously used a value such as `yes` to mean write mode, it now means read-only: use `false`.
</Note>
