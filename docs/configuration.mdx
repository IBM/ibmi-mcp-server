...
| `IBMI_EXECUTE_SQL_PARSE_VALIDATION` | When to run `QSYS2.PARSE_STATEMENT` wire validation. `auto` skips the round trip when the in-process parser classified the statement; `always` runs it for every query (strict/audit mode). Unrecognized values fall back to `auto` | `auto` | `auto`, `always` |
