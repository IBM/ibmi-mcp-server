...
**Solution**: Either **omit these variables entirely**, or set them to placeholder values like `TOKEN_AUTH`.
...
1. Reads your IBM i credentials from the process environment
...
|--------|-------|-------------|---------|
| `--user` | `-u` | IBM i username | `DB2i_USER` |
| `--password` | `-p` | IBM i password | `DB2i_PASS` |
| `--host` | `-h` | IBM i host address | `DB2i_HOST` |
...
<AccordionGroup>
  <Accordion title="Validation Error: DB2i_USER and DB2i_PASS Required">
    ...
    **Solution**: Either completely omit these variables, or set them to placeholder values like `TOKEN_AUTH`. Do not use empty strings (`DB2i_USER=""`).
  ...
  <Accordion title="Encryption Key Errors">
    ...
    **Solutions:**
    - Verify `IBMI_AUTH_PRIVATE_KEY_PATH` and `IBMI_AUTH_PUBLIC_KEY_PATH` are correct
