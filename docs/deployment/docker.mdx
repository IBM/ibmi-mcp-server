...
Docker and Podman can inject variables into the process with `--env-file`. This is orchestrator injection, not the server reading a dotenv file:
...
# Configure environment (Compose injects these into the container process)
cp .env.example .env
