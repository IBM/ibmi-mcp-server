...
<Note>
**Important**: You'll need port 8076 when configuring the `DB2i_PORT` environment variable. Ensure your IBM i firewall allows inbound connections on this port.
</Note>
...
Once Mapepire is running, set IBM i MCP Server connection variables:

```bash
export DB2i_HOST=your-ibmi-hostname
