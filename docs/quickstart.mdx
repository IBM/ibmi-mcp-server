...
<Tabs>
  <Tab title="HTTP Transport (Recommended)">
    **Best for**: Web applications, remote connections, development with browser tools
    ...
    **Session Modes (HTTP Only)**:
    - `auto` (default): Automatically detects client capabilities
  ...
  <Tab title="Stdio Transport">
    **Best for**: CLI tools, MCP Inspector, direct process communication
