...
Start the server in HTTP mode for the best experience with AI clients:
...
<Tabs>
  <Tab title="NPM Package (Recommended)">
    ...
    **Session Modes**:
    ...
    Set via environment: `MCP_SESSION_MODE=stateful npx @ibm/ibmi-mcp-server@latest -y --transport http`
  </Tab>
