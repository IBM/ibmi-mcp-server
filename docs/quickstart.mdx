...
Set your IBM i connection details in the process environment:
...
<Note>
**Local development only:** To load these from a file, set `MCP_SERVER_CONFIG` to that path. The server does not read `.env` unless this variable is set.
</Note>
...
- `DB2i_HOST`, `DB2i_USER`, and `DB2i_PASS` are set in the process environment
...
<Steps>
  <Step title="Sources">
    Defines the IBM i data sources your tools will connect to. The environment variables (`${DB2i_HOST}`, etc.) are replaced with values from the process environment.
...
3. IBM i credentials are set in the process environment
...
<AccordionGroup>
  ...
  <Accordion title="Port Already in Use" icon="">
    Change the port in the process environment:
    ```bash
    export MCP_HTTP_PORT=3011
    ```
