...
claude mcp add ibmi-mcp \
  ...
  -- npx @ibm/ibmi-mcp-server@latest --tools /absolute/path/to/tools
```
...
Edit `.mcp.json` in your home directory:
...
npx @ibm/ibmi-mcp-server@latest --list-toolsets --tools /path/to/tools
```
...
<AccordionGroup>
  ...
  <Accordion title="Tools Not Loading">
    ...
    **Solutions**:
    ...
    - Test tools path: `npx @ibm/ibmi-mcp-server@latest --list-toolsets --tools /path`
